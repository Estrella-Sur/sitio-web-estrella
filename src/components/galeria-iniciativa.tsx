...
interface GaleriaIniciativaProps {
  initiativeId: string;
}

export function GaleriaIniciativa({ initiativeId }: GaleriaIniciativaProps) {
  ...
  const fetchGaleria = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/methodologies/${initiativeId}/galeria`);
      ...
      setIniciativa(data.methodology);
                ...
                {images.map((image, index) => (
                  <Card key={image.id} className="overflow-hidden hover:shadow-lg transition-shadow group cursor-pointer" onClick={() => openModal(index)}>
                    ...
                    >
                      <Image
                        src={image.imageUrl}
                        alt={image.imageAlt || image.title || 'Imagen de la iniciativa'}
                        fill
            ...
            <Card className="h-full">
              <CardContent className="p-12 text-center h-full flex flex-col justify-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Esta iniciativa aún no tiene imágenes en su galería.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="relative max-w-7xl max-h-full w-full h-full flex items-center justify-center">
            ...
            >
              <Image
                ...
                alt={images[selectedImageIndex].imageAlt || images[selectedImageIndex].title || 'Imagen de la iniciativa'}
                width={1200}
                height={800}
