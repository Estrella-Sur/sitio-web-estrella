...
export function GaleriaProyecto({ projectId }: GaleriaProyectoProps) {
  ...
  const fetchGaleria = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/projects/${projectId}/galeria`);
                ...
                {images.map((image, index) => (
                  <Card key={image.id} className="overflow-hidden hover:shadow-lg transition-shadow group cursor-pointer" onClick={() => openModal(index)}>
                    ...
                    >
                      <Image
                        src={image.imageUrl}
                        alt={image.imageAlt || image.title || 'Imagen del proyecto'}
                        fill
            ...
            <Card className="h-full">
              <CardContent className="p-12 text-center h-full flex flex-col justify-center">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  Este proyecto aún no tiene imágenes en su galería.
                </p>
              </CardContent>
            </Card>
          )}
          ...
          <div className="relative max-w-7xl max-h-full w-full h-full flex items-center justify-center">
            ...
            >
              <Image
                ...
                alt={images[selectedImageIndex].imageAlt || images[selectedImageIndex].title || 'Imagen del proyecto'}
                width={1200}
                height={800}
