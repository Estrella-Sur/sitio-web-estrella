...
export function ConvocatoriasManagement() {
  ...
  const fetchConvocatorias = async (page: number = 1, search: string = '', status: string = 'ALL', sort: string = 'createdAt', order: string = 'desc') => {
    try {
      ...
      // Resetear los valores de los selects cuando se cargan nuevas convocatorias
      const newSelectValues: Record<string, string> = {};
      (data.convocatorias || []).forEach((conv: Convocatoria) => {
        newSelectValues[conv.id] = conv.status;
      });
  ...
  const updateConvocatoriaStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Actualizar el valor del select localmente
        setSelectValues(prev => ({
          ...prev,
          [changingStatus.convocatoria.id]: changingStatus.newStatus
        }));
        fetchConvocatorias(currentPage, searchTerm, statusFilter, sortBy, sortOrder);
      ...
      <Tabs defaultValue="all" onValueChange={handleTabChange}>
                ...
                <Card key={convocatoria.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      <div className="flex gap-2">
                        <Select
                          value={selectValues[convocatoria.id] || convocatoria.status}
                          onValueChange={(value) => {
                            if (value !== (selectValues[convocatoria.id] || convocatoria.status)) {
                              // Actualizar el valor visual del select
                              setSelectValues(prev => ({
                                ...prev,
                                [convocatoria.id]: value
                              }));
                              // Abrir el diálogo de confirmación
                              setChangingStatus({convocatoria, newStatus: value});
                            }
                          }}
                        >
        ...
        <TabsContent value="active" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">No se encontraron convocatorias activas</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {convocatorias.map((convocatoria) => (
        ...
        <TabsContent value="upcoming" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">No se encontraron convocatorias próximas</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {convocatorias.map((convocatoria) => (
        ...
        <TabsContent value="closed" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">No se encontraron convocatorias cerradas</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {convocatorias.map((convocatoria) => (
        ...
        <TabsContent value="draft" className="space-y-4">
          {loading ? (
            ...
            <div className="text-center py-12">
              <p className="text-muted-foreground">No se encontraron borradores</p>
            </div>
          ) : (
            <div className="grid gap-4">
              {convocatorias.map((convocatoria) => (
          ...
          <DialogContent>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => {
                // Restaurar el valor original del select si se cancela
                setSelectValues(prev => ({
                  ...prev,
                  [changingStatus.convocatoria.id]: changingStatus.convocatoria.status
                }));
                setChangingStatus(null);
              }}>
                Cancelar
